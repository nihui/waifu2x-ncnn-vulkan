...
static int optind = 1;
static wchar_t getopt(int argc, wchar_t *const argv[],
        ...
        if (pixeldata) {
          ...
          if (pixeldata) {
            ...
            if (c == 1) {
              ...
              pixeldata =
                  stbi_load_from_memory(filedata, length, &w, &h, &c, 3);
              c = 3;
            } else if (c == 2) {
              // grayscale + alpha -> rgba
              stbi_image_free(pixeldata);
              pixeldata =
                  stbi_load_from_memory(filedata, length, &w, &h, &c, 4);
              c = 4;
            }
          }
#endif // _WIN32
...
#if _WIN32
      success =
          wic_encode_jpeg_image(v.outpath.c_str(), v.outimage.w, v.outimage.h,
                                v.outimage.elempack, v.outimage.data);
#else
...
#endif
{
  ...
  std::vector<path_t> output_files;
  {
    if (path_is_directory(inputpath) && path_is_directory(outputpath)) {
...
#if _WIN32
          fwprintf(
              stderr, L"both %ls and %ls output %ls ! %ls will output %ls\n",
      ...
      fprintf(stderr, "inputpath and outputpath must be either file or "
  ...
  } else if (model.find(PATHSTR("models-upconv_7_anime_style_art_rgb")) !=
             path_t::npos) {
    prepadding = 7;
  ...
  if (noise == -1) {
    ...
    swprintf(parampath, 256, L"%s/noise%d_scale2.0x_model.param", model.c_str(),
             noise);
    swprintf(modelpath, 256, L"%s/noise%d_scale2.0x_model.bin", model.c_str(),
             noise);
  }
#else
  char parampath[256];
  ...
  if (noise == -1) {
    ...
    sprintf(parampath, "%s/noise%d_scale2.0x_model.param", model.c_str(),
            noise);
